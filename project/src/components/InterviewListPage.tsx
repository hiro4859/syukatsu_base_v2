...
import { supabase, Interview, Template, Company } from '../lib/supabase';
import { ChevronDown, ChevronUp, Copy, Check, Save, MessageSquare, Trash2, ArrowUpDown, Filter } from 'lucide-react';
...
export default function InterviewListPage() {
  ...
  const loadData = async () => {
    ...
    const [templatesResult, interviewsResult, companiesResult] = await Promise.all([
      supabase
        .from('templates')
        .select('*')
        .eq('user_id', user.id)
    ...
    if (interviewsResult.data && companiesResult.data) {
      const companyMap = new Map(companiesResult.data.map(c => [c.id, c.name]));
      const interviewsWithCompany = interviewsResult.data.map(interview => ({
        ...interview,
        company_name: companyMap.get(interview.company_id) || '不明な企業'
      }));
      setInterviews(interviewsWithCompany);
  ...
  const toggleExpand = (type: 'template' | 'interview', id: string) => {
  ...
  const startEdit = (type: 'template' | 'interview', id: string, theme: string, content: string) => {
  ...
  const deleteItem = async (type: 'template' | 'interview', id: string) => {
    if (!confirm('本当に削除しますか？')) return;

    const table = type === 'template' ? 'templates' : 'interviews';
    const { error } = await supabase
      .from(table)
      .delete()
      .eq('id', id);
  ...
  const groupedInterviews = sortedCompanies.reduce((acc, company) => {
    const companyInterviews = interviews.filter(interview => interview.company_id === company.id);
    if (companyInterviews.length > 0) {
      acc[company.id] = {
        company,
        entries: companyInterviews
      };
    }
    return acc;
  }, {} as Record<string, { company: Company; entries: InterviewWithCompany[] }>);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 text-lg">ログインして面接一覧を表示</p>
        </div>
      </div>
    );
  }

  const renderInterviewItem = (
    type: 'template' | 'interview',
    id: string,
    theme: string,
    content: string,
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-slate-800">面接一覧</h1>
          </div>
          <p className="text-slate-600">
            テンプレートと各企業の面接台本を管理します
          </p>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                {templates.map(template =>
                  renderInterviewItem('template', template.id, template.theme, template.content)
                )}
              </div>
            </div>
          )}
          ...
          {Object.keys(groupedInterviews).length > 0 && (
            <div>
              ...
              <div className="space-y-6">
                {Object.values(groupedInterviews).map(({ company, entries }) => (
                  <div key={company.id}>
                    ...
                    <div className="space-y-3 ml-4">
                      {entries.map(entry =>
                        renderInterviewItem('interview', entry.id, entry.theme, entry.script, entry.company_name)
                      )}
                    </div>
                  </div>
                ))}
          ...
          {templates.length === 0 && Object.keys(groupedInterviews).length === 0 && (
